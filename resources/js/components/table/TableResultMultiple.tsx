...
function TableResultTypeMultiple({ scoringType, contest, genderCategory }: { scoringType: string; contest: Contest; genderCategory: string[] }) {
    ...
    // Make unique based on criteria + participant_id + participant_gender
        ...
        if (lower === 'malefemale') {
            return (
                <div className="flex flex-col gap-8 2xl:flex-row">
                    {['Male', 'Female'].map((gender) => renderSingleTable(gender, data, criteria))}
                </div>
            );
        }
