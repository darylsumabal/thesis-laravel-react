...
type PROPS = {
    scoringType: 'Point Based Single Round' | 'Point Based Multiple Round' | 'Rank Based Single Round' | 'Rank Based Multiple Round';
    contest: Contest;
};

function TableRankedFinal({ scoringType, contest }: PROPS) {
    const { tableRankedFinal, qualified } = usePage().props;

    const data: JudgeScoreTest[] = tableRankedFinal?.flatMap((r) => r.judges_score) ?? [];
    ...
    const scoringPoint = ['Point Based Single Round', 'Point Based Multiple Round'];

    const scoringRank = ['Rank Based Single Round', 'Rank Based Multiple Round'];
    return (
...
export default TableRankedFinal;
